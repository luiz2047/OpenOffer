...
> Legacy archive note: this client/relay integration writeup describes the pre-OpenOffer hosted-control-plane era. OpenOffer ships local/self-hosted only; hosted defaults are intentionally absent.
...
`controlPlaneBaseUrl` is user- or package-configured; there is no hosted default in OpenOffer. `content-type: application/json`. Body:
...
- `connectUrl()` returns the configured `BACKEND_URL`; OpenOffer keeps no hosted default.
