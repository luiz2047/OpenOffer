...
|---|---|---|---|---|---|---|---|
| **Configured control-plane** | yes | multipart/form-data; image first resized with Sharp to ≤768 px on long edge, base64 encoded | no — only forwards | yes (backend treats as multimodal) | server-side fallback | medium (cloud) | `generateWithNatively` `LLMHelper.ts:1698`; `streamWithNatively` |
...
- **No data-scope assertion** runs for this path
  (compare to `LLMHelper.ts:1699` for the control-plane path). A screenshot can leave the
...
the default in the multimodal stream list (after the control-plane/Codex/OpenAI),
...
layer still applies because the control-plane path is short-circuited for
