...
**App version:** 2.6.0
**Goal:** Make OpenOffer behave like a real, Developer ID-signed + notarized production macOS desktop app — not a hacked ad-hoc local build. Cover signing/notarization, permissions/TCC, overlay/visibility toggle reliability, audio capture stability, feature gating, startup performance, and UX reliability.
...
| `src/components/settings/NativelyApiSettings.tsx` | Control-plane key UI | Key entry/validation/redaction. |
...
### 2.2 Feature gating — STRUCTURE MAPPED (audit pending Phase 2)
...
| 2 | Feature gating | ✅ F4 fixed + tested (money path otherwise sound; F5/F7 UX follow-ups) | 2026-05-29 |
