...
| `electron/services/__tests__/NativelyApiE2E.test.mjs` | real control-plane smoke (env-gated; entire suite skipped by default) | 0 reported (suite-level skip) |
| **Total new tests** | | **86** |
...
| 2026-05-15 12:05 | `RUN_NATIVELY_API_E2E=1 NativelyApiE2E.test.mjs` (real control-plane) | 3 | 3 | 0 | 0 | After fixing FINDING-011 (wrong base URL + auth header) |
| 2026-05-15 12:07 | `modes-live-response-eval.ts` (live control-plane baseline) | 45 | 40-45 | 0-5 | 0 | 0 creator-name leaks; remaining failures are over-narrow / over-broad harness regexes, not real prompt regressions |
| 2026-05-15 12:50 | `modes-live-response-eval.ts` post-Phase-4 (live control-plane baseline) | 45 | 44 | 1 | 0 | 97.8% pass; only fail is FINDING-012 (real LLM bug, harness correctly caught it). 0 creator-name leaks. By mode: general 7/7, sales 7/7, recruiting 7/7, team-meet 6/6, looking-for-work 6/6, lecture 6/6, technical-interview 5/6 |
...
**Real bug found and fixed:** FINDING-011 — `electron/services/__tests__/NativelyApiE2E.test.mjs` used the wrong base URL (`api.natively.app`, NXDOMAIN) and wrong auth header style (`Authorization: Bearer`). The corrected run uses the configured control-plane URL and `x-natively-key`. After fix the smoke test resolves and returns 404 for `/v1/health` (route is intentionally absent), which the test accepts. Fix documented in BUGFIX_LOG.
...
- `RUN_NATIVELY_API_E2E=1` was set only for the targeted control-plane smoke run with the provided key passed via `NATIVELY_API_KEY` env var. Key is never written to a log or committed.
- No new product code changed in this run except the test-file URL/header fix.

## Known limitations

* Real control-plane tests are env-gated behind `RUN_NATIVELY_API_E2E=1`. They are **skipped** by default to avoid burning trial quota and to keep CI hermetic. The skip path is exercised every run.
...
* **Top 10 fixes still needed:**
  ...
  5. **FINDING-005 (low)** — control-plane smoke test exists but is suite-skipped by default; add a CI lane that runs it weekly with a dedicated test trial token to surface API regressions early.
