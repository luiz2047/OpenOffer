...
### FINDING-011 — Real control-plane smoke test hits an unresolved domain with the wrong auth header
...
- **Expected behavior:** The test must hit the same configured base URL `LLMHelper.ts:1709` uses and send the same auth header style (`x-natively-key: <key>`). A 404 from `/v1/health` is an accepted outcome because the route is intentionally absent — the goal is connectivity + auth, not health.
...
- **Root cause:** Drift between the test fixture defaults (`api.natively.app`, `Authorization: Bearer`) and the configured base URL/headers (`x-natively-key`) defined in `electron/LLMHelper.ts:1709-1720`.
...
- **Fix applied:**
  ...
  + const API_BASE = process.env.NATIVELY_API_BASE ?? process.env.CONTROL_PLANE_URL;
  ...
...
### FINDING-012 — control-plane LLM emits buggy two-sum (`complement = target, num` tuple) intermittently

- **Mode affected:** technical-interview (and any custom mode that surfaces a two-sum-style problem through the control-plane).
