# Installing This Local-STT OpenOffer Build

This guide is for running the OpenOffer source tree that includes the new `Local STT` provider. Your already-installed OpenOffer app will not have this feature until you build and run this modified project or package a new installer from it.
...
```powershell
cd C:\path\to\openoffer-cluely-ai-assistant
```
...
## 4. Run OpenOffer in Development Mode
...
## 5. Configure Local STT in OpenOffer
...
When `Test Connection` says connected, start a meeting or audio capture flow. OpenOffer opens separate local STT streams for system audio and microphone, so WhisperLive must allow at least two clients.
...
- OpenOffer does not start, install, or supervise WhisperLive. WhisperLive must be running before you use Local STT.
