...
const UpdateBanner: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (!api) return;
        let cancelled = false;
        const canAutoUpdatePromise = api.getCanAutoUpdate?.();
        if (!canAutoUpdatePromise) return () => { cancelled = true; };
        canAutoUpdatePromise
    ...
    const handleInstall = async () => {
        ...
        if (window.electronAPI?.platform === 'darwin') {
            ...
            try {
                ...
                const url = `https://github.com/luiz2047/openoffer/releases/download/v${version}/OpenOffer-${version}-${dmgSuffix}.dmg`;
