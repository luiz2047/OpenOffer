 ...
 *        -> chrome.tabs.sendMessage('openoffer:extract')  (token NEVER crosses this)
...
export type DomPostOutcome =
  ...
  | { kind: 'no-session' } // 409 — OpenOffer running but no active session/overlay
...
export async function postDomToDesktop(
  ...
  switch (res.status) {
    ...
    case 409:
      // OpenOffer is running and paired, but no active session/overlay to receive
      // the context. The user must start a OpenOffer session, then capture again.
...
async function extractFromTab(tabId: number): Promise<ExtractedTab> {
  ...
  const response = (await chrome.tabs.sendMessage(tabId, { type: 'openoffer:extract' })) as
 ...
 * OpenOffer hotkey fires, Chrome is NOT the focused OS app, so `currentWindow`
...
// looking at the OpenOffer overlay. Capture is now triggered by a NATIVELY global
...
try { chrome.alarms.create('openoffer-ws-keepalive', { periodInMinutes: 0.5 }); } catch (_) {}
chrome.alarms.onAlarm.addListener((alarm) => {
  if (alarm.name === 'openoffer-ws-keepalive') void ensureWsConnected();
});
...
// OpenOffer hotkey the capture channel is already live — closing the MV3 idle-death
