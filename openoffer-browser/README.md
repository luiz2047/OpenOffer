# OpenOffer Page Context — companion browser extension

A minimal, privacy-correct Manifest V3 extension that sends the **active tab's
readable content** to your local OpenOffer desktop app, **once, on demand**. It
never runs in the background, never auto-captures, and only ever talks to your
own machine over loopback.

It is part of the [OpenOffer](../) monorepo and is licensed **AGPL-3.0-only**,
same as the desktop app. Source lives in this directory (`openoffer-browser/`).

## How it works

Capture is triggered from the **OpenOffer desktop app** (a global hotkey that works
from any focused app — including while you're looking at the OpenOffer overlay, not
...
```
OpenOffer desktop hotkey (default ⌘/Ctrl+Shift+Y, owned by the desktop app)
...
4. Click **Load unpacked** → select `openoffer-browser/dist`
...
1. In OpenOffer desktop: **Settings → Phone Mirror** → enable, then in the
   **Browser Extension** card click **Connect browser extension** (opens a 60s
   window).
2. Click the extension's toolbar icon → **Connect to OpenOffer**. Done — a green
...
- **From the desktop (primary):** press the OpenOffer capture hotkey
  (**`⌘/Ctrl+Shift+Y`** by default, configurable in OpenOffer's keybindings). This
  works from any focused app — including the OpenOffer overlay. If the browser
...
- **"Pairing expired"** (amber dot): you clicked **Rotate token** in Settings (the
  deliberate security reset). Re-pair via **Connect to OpenOffer**.
- **"Open OpenOffer and enable Phone Mirror"** (red dot): Phone Mirror is off. The
