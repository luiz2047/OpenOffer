/**
 * ================================================================================
 * InstallPingManager - Optional Anonymous Install Counter
 * ================================================================================
 *
 * PURPOSE:
 * This module can send a ONE-TIME anonymous ping when explicitly configured.
 * OpenOffer ships with this disabled so the default build makes no project
 * telemetry requests.
 *
 * WHAT IS SENT (exactly):
 * - "app": "openoffer" (hardcoded app identifier)
 ...
 *
 * This is NOT analytics. This is NOT telemetry. This is a disabled-by-default
 * install counter for maintainers who self-host an endpoint.
 * ================================================================================
 */
...
/**
 * Anonymous install ping endpoint. Empty by default to preserve local-first
 * behavior; self-hosted builds may set OPENOFFER_INSTALL_PING_URL.
 */
const INSTALL_PING_URL = process.env.OPENOFFER_INSTALL_PING_URL || '';
...
export async function sendAnonymousInstallPing(): Promise<void> {
    try {
        if (!INSTALL_PING_URL) {
            console.log('[InstallPingManager] Install ping disabled');
            return;
        }
