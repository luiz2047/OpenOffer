...
const SENSITIVE_KEY_RE = /(api[_-]?key|authorization|bearer|token|secret|password|credential|raw[_-]?(transcript|prompt|reference|content|query|resume|jd|persona|negotiation)|transcript(text)?|prompt|reference(content)?|evidence(text)?|screenshot(path)?|image(path)?|error(body|response|message)?|responsebody|body|query(text|string)?|user(input|message)|chunk(text|content)?|snippet(text)?|resume(text)?|persona(text)?|negotiation(text|script|context)?|jd(text|content)?|customcontext|customnotes|notes|note|answer(text)?|question(text)?|latestquestion|salary|compensation|interview(prep|dossier|vacancy|retro|question|context|notes?|text)|prep(brief|notes?|context)?|vacancy(dossier|text|context)?|retro(notes?|text|context)?|content|text)$/i;
...
const REMOVE_VALUE_KEY_RE = /(raw[_-]?(transcript|prompt|reference|content|query|resume|jd|persona|negotiation)|transcript(text)?|prompt|reference(content)?|evidence(text)?|screenshot(path)?|image(path)?|error(body|response)?|responsebody|body|query(text|string)?|user(input|message)|chunk(text|content)?|snippet(text)?|resume(text)?|persona(text)?|negotiation(text|script|context)?|jd(text|content)?|customcontext|customnotes|notes|note|answer(text)?|question(text)?|latestquestion|interview(prep|dossier|vacancy|retro|question|context|notes?|text)|prep(brief|notes?|context)?|vacancy(dossier|text|context)?|retro(notes?|text|context)?|content|text)$/i;
    ...
    try {
      ...
      if (sink.name === 'posthog') {
        ...
        post(`${host}/capture/`, {
          ...
          body: JSON.stringify({
            api_key: apiKey,
            event: record.name,
            distinct_id: sink.distinctId || 'openoffer-desktop',
            properties: {
              ...record.properties,
      ...
      if (sink.name === 'sentry') {
        ...
        const event = {
          ...
          platform: 'node',
          level: 'error',
          logger: 'openoffer-desktop',
          release: sink.release || 'unknown',
          environment: sink.environment || 'production',
          tags: { service: 'openoffer-desktop', event: record.name, ...(record.provider ? { provider: record.provider } : {}) },
...
export function parseClientSentryDsn(dsn: string | undefined): ParsedSentryDsn | null {
  ...
  try {
    ...
    return {
      ...
      authHeader: `Sentry sentry_version=7, sentry_client=openoffer-desktop/1.0, sentry_key=${publicKey}`,
    };
  } catch {
    return null;
  }
