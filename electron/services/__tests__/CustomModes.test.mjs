...
const CUSTOM_MODES = {
  support: {
    ...
    sentinels: {
      ...
      audio: 'tccutil reset Microphone com.electron.openoffer',
  ...
  'sales-demo': {
    ...
    templateType: 'sales',
    customContext:
      'You are a product demo copilot for OpenOffer. Focus on product value, workflow fit, pricing, objections, and honest limitations. Do not claim enterprise certifications or features not present in files.',
    files: [
    ...
    sentinels: {
      ...
      security: 'OpenOffer does not currently hold SOC2 Type 2',
...
describe('Custom mode 1: Customer Support Escalation', () => {
  ...
  test('scenario 2: mic not working → retrieves audio troubleshooting', () => {
    const ctx = runCustom({
      ...
      transcript: 'Customer reports they cannot record any audio in OpenOffer after a system update.',
    });
...
describe('Custom mode 5: Sales Demo / Product Specialist', () => {
  test('scenario 1: prospect asks pricing → retrieves pricing policy', () => {
    const ctx = runCustom({
      folder: 'sales-demo',
      query: 'Pro tier price annual monthly user discount OpenOffer pricing tiers',
      transcript: 'Prospect comparing Pro tier annual price and monthly price for OpenOffer. They want to know the discount on annual, the user limits, and whether Free tier supports reference files.',
    });
  ...
  test('scenario 2: compares to Cluely → retrieves feature matrix', () => {
    const ctx = runCustom({
      folder: 'sales-demo',
      query: 'OpenOffer Cluely Final Round Otter feature comparison per mode reference files local retrieval',
      transcript: 'Prospect uses Cluely and wants a feature matrix comparing OpenOffer vs Cluely vs Final Round AI vs Otter on per-mode reference files, custom modes, and hybrid local retrieval.',
    });
...
const DELETION_PROBE = {
  support: { sentinel: 'tccutil reset Microphone com.electron.openoffer', owner: 'support_audio_troubleshooting.txt' },
