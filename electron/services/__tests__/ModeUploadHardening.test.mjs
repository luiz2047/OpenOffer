...
describe('FIX-009: modes:upload-reference-file hardening', () => {
  ...
  test('public OpenOffer build does not put reference uploads behind a commercial gate', () => {
    assert.equal(body.includes('isProOrTrialActive()'), false);
    assert.equal(body.includes('pro_required'), false);
    assert.ok(body.includes('showOpenDialog'), 'Upload handler must still open the local file picker');
  });
});
