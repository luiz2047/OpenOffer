...
// exchange and refresh are proxied through an explicit calendar proxy, which
// holds the secret.
...
const CALENDAR_PROXY_URL = (
    process.env.OPENOFFER_CALENDAR_PROXY_URL ||
    process.env.CALENDAR_PROXY_URL
)?.trim().replace(/\/+$/, '') || null;
...
export class CalendarManager extends EventEmitter {
    ...
    public isProxyConfigured(): boolean {
    ...
    private getCalendarProxyBaseUrl(): string {
        if (!CALENDAR_PROXY_URL) {
            throw new Error('Calendar is disabled until an explicit proxy URL is configured.');
        }
        return CALENDAR_PROXY_URL;
    }
            ...
            const server = http.createServer(async (req, res) => {
                try {
                    if (req.url?.startsWith('/auth/callback')) {
                        ...
                        if (code) {
                            res.end('Authentication successful! You can close this window and return to OpenOffer.');
    ...
    private async exchangeCodeForToken(code: string) {
        try {
            // Proxied through the configured calendar proxy so GOOGLE_CLIENT_SECRET never ships in the desktop app.
            // Fetch (vs. axios) so this call shares the global keep-alive pool with every other
            // request to the configured proxy and exposes the same error shape (res.ok / res.status)
            // as the rest of the codebase.
            const response = await fetch(`${this.getCalendarProxyBaseUrl()}/api/calendar/exchange`, {
                method: 'POST',
    ...
    private async refreshAccessToken() {
        ...
        try {
            // Proxied through the configured calendar proxy so GOOGLE_CLIENT_SECRET never ships in the desktop app.
            const response = await fetch(`${this.getCalendarProxyBaseUrl()}/api/calendar/refresh`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    private showNotification(event: CalendarEvent) {
        ...
        const notif = new Notification({
            ...
            body: `"${event.title}" starts in 2 minutes. Start OpenOffer?`,
            actions: [
