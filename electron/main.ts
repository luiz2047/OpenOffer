...
const getLogFile = (): string | null => {
  ...
  try {
    _logFile = path.join(app.getPath('documents'), 'openoffer_debug.log');
    return _logFile;
  } catch {
...
function formatPermissionMessage(reason: PermissionReason, extra?: { device?: string }): string {
  ...
  switch (reason) {
    ...
    case 'mac-screen-recording-restricted':
      ...
      return 'Screen Recording is restricted by device policy. Interviewer audio will not be captured. Contact your administrator to allow screen capture for OpenOffer.';
    case 'mac-screen-recording-revoked-rebuild':
      ...
      if (!isMac) return formatPermissionMessage('system-audio-stuck');
      return 'System audio is being captured but every sample is silent. This usually means macOS Screen Recording permission needs to be re-granted to this build of OpenOffer. Open System Settings → Privacy & Security → Screen Recording, toggle OpenOffer off and back on, then restart the app. (If you recently rebuilt or updated, the previous grant may not apply.)';
    case 'mic-denied':
      return isMac
        ? 'Microphone access denied. Please allow microphone access in System Settings → Privacy & Security → Microphone, then restart OpenOffer.'
        : 'Microphone access denied. Please allow microphone access in Settings → Privacy → Microphone, then restart OpenOffer.';
    case 'mic-zero-fill':
      return isMac
        ? 'Microphone is producing silent audio. Check that the device is unmuted and that macOS Microphone permission is granted to OpenOffer in System Settings → Privacy & Security → Microphone.'
        : 'Microphone is producing silent audio. Check that the device is unmuted and that OpenOffer has microphone access in Settings → Privacy → Microphone.';
...
export class AppState {
  ...
  // launcher's "Meeting ongoing" pill switches back to "Start OpenOffer" the
  ...
  private _stageHeartbeatTimer: NodeJS.Timeout | null = null;
  private _stageHeartbeatMeetingId: string | null = null;
  ...
  private setupAutoUpdater(): void {
    const updateConsentPolicyVersion = '2026-07-14-stage-workspace'
    ...
    // Stable is the safe default. Maintainer preview builds opt in explicitly
    // through OPENOFFER_UPDATE_CHANNEL=preview and may consume prereleases.
    const updateChannel = process.env.OPENOFFER_UPDATE_CHANNEL === 'preview' ? 'preview' : 'latest';
    autoUpdater.channel = updateChannel;
    (autoUpdater as any).allowPrerelease = updateChannel === 'preview';
    console.log(`[AutoUpdater] Channel: ${autoUpdater.channel} (allowPrerelease=${updateChannel === 'preview'})`)
    ...
    setTimeout(() => {
      if (process.env.NODE_ENV === "development") {
        ...
        try {
          ...
          consent = (settings.get('updateChecksConsent') ?? false)
            && settings.get('updateChecksConsentPolicyVersion') === updateConsentPolicyVersion;
        } catch { /* fail closed */ }
        if (!consent) {
          console.log('[AutoUpdater] Background update check skipped: explicit update consent is disabled');
          return;
        }
  ...
  private async checkForUpdatesManual(): Promise<void> {
    try {
      ...
      const notes = await releaseManager.fetchReleaseNotes(
        process.env.OPENOFFER_UPDATE_CHANNEL === 'preview' ? 'preview' : 'latest',
      );
    ...
    try {
      ...
      if (screenCapability.effectiveDenied) {
        ...
        try {
          ...
          this.sendAudioCaptureFailed({
            channel: 'system',
            message: 'System audio capture failed to initialize. The native audio module could not allocate the capture device. Restarting OpenOffer may help; if the problem persists, file a bug.',
            attempt: 0,
            maxAttempts: 0,
            terminal: true,
            stuck: false,
      ...
      if (!this.microphoneCapture) {
        ...
        try {
          ...
          this.sendAudioCaptureFailed({
            channel: 'mic',
            message: 'Microphone capture failed to initialize. The native audio module could not open the default input device. Check that the device is connected and not in exclusive use by another app, then restart OpenOffer.',
            attempt: 0,
            maxAttempts: 0,
            terminal: true,
            stuck: false,
  ...
  private startStageHeartbeat(meetingId: string): void {
    this.stopStageHeartbeat();
    this._stageHeartbeatMeetingId = meetingId;
    const beat = () => {
      try {
        const db = DatabaseManager.getInstance().getDb();
        db?.prepare(`UPDATE stage_sessions SET heartbeat_at = ?, updated_at = ? WHERE meeting_id = ? AND status IN ('initializing', 'recording')`).run(Date.now(), new Date().toISOString(), meetingId);
      } catch { /* recovery on restart remains authoritative */ }
    };
    beat();
    this._stageHeartbeatTimer = setInterval(beat, 5_000);
  }

  private stopStageHeartbeat(): void {
    if (this._stageHeartbeatTimer) clearInterval(this._stageHeartbeatTimer);
    this._stageHeartbeatTimer = null;
    this._stageHeartbeatMeetingId = null;
  }

  public stopStageHeartbeatFor(meetingId: string): void {
    if (this._stageHeartbeatMeetingId === meetingId) this.stopStageHeartbeat();
  }
  ...
  public async startMeeting(metadata?: any): Promise<void> {
    ...
    if (!(await ensureMacMicrophoneAccess('meeting start'))) {
      ...
      // a denied/revoked mic grant made "Start OpenOffer" do nothing on screen.
    ...
    if (metadata) {
      ...
      if (typeof metadata.stageWorkspaceMeetingId === 'string') this.startStageHeartbeat(metadata.stageWorkspaceMeetingId);
    }
  ...
  public async endMeeting(): Promise<void> {
    ...
    // "Meeting ongoing" pill reverts to "Start OpenOffer" immediately;
  ...
  public showTray(): void {
    ...
    this.tray.setToolTip('OpenOffer') // This tooltip might also need update if we change global shortcut, but global shortcut is removed.
    this.updateTrayMenu();
  ...
  public updateTrayMenu() {
    ...
    this.tray.setToolTip('OpenOffer');
  ...
  private _applyDisguise(mode: 'terminal' | 'settings' | 'activity' | 'none'): void {
    ...
    switch (mode) {
      ...
      case 'none':
        ...
        if (isMac) {
          iconPath = app.isPackaged
            ? path.join(process.resourcesPath, "openoffer.icns")
...
async function initializeApp() {
  ...
  // Release CI smoke: prove the packaged main process can initialize without
  // creating windows, requesting permissions, or contacting a provider.
  // The runner owns the timeout and terminates the process after this point.
  if (process.env.OPENOFFER_PACKAGED_SMOKE === '1') {
    app.exit(0);
    return;
  }
  ...
  // Bind TelemetryService after app.whenReady. Telemetry is explicit opt-in:
  // a clean launch must not write or transmit analytics before consent.
  try {
    const { telemetryService } = require('./services/telemetry/TelemetryService');
    ...
    if (telemetryEnabledSetting === true) try {
    ...
    const sinks: Array<Record<string, unknown>> = [{ name: 'local-jsonl', enabled: telemetryEnabledSetting === true }];
    if (telemetryEnabledSetting === true && process.env.POSTHOG_API_KEY) {
    ...
    if (telemetryEnabledSetting === true && process.env.SENTRY_DSN) {
      sinks.push({ name: 'sentry', enabled: true, dsn: process.env.SENTRY_DSN, release, environment });
    }
    if (telemetryEnabledSetting === true && process.env.AXIOM_TOKEN && process.env.AXIOM_DATASET) {
      sinks.push({ name: 'axiom', enabled: true, apiKey: process.env.AXIOM_TOKEN, dataset: process.env.AXIOM_DATASET });
    }

    telemetryService.configure({
      userDataPath,
      enabled: telemetryEnabledSetting === true,
      localEnabled: telemetryEnabledSetting === true,
      sinks,
    });
      ...
      try {
        ...
        try {
          ...
          if (micStatus === 'denied') {
            ...
            appState.sendAudioCaptureFailed({
              channel: 'mic',
              message: 'Microphone is restricted by device policy. Contact your administrator to enable microphone access for OpenOffer.',
              attempt: 0,
              maxAttempts: 0,
              terminal: true,
              stuck: false,
