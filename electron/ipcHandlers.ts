...
async function checkGigaSTTRuntime(): Promise<{
  ...
  if (!binary.found) {
    ...
    state = 'not-running';
    label = 'Not running';
    message = 'GigaSTT server is not running. OpenOffer will retry starting it when a meeting starts.';
  } else if (ready.ok) {
    state = 'ready';
    label = 'Ready';
...
export function initializeIpcHandlers(appState: AppState): void {
  ...
  safeHandle('get-log-file-path', async () => {
    try {
      return path.join(app.getPath('documents'), 'openoffer_debug.log');
    } catch {
      return null;
    }
  });
  ...
  safeHandle('open-log-file', async () => {
    try {
      const logPath = path.join(app.getPath('documents'), 'openoffer_debug.log');
  ...
  safeHandle('repair-tcc-permissions', async () => {
    ...
    return {
      ...
      message: anyOk
        ? 'Permissions reset. Quit OpenOffer completely (Cmd+Q) and reopen. macOS will ask you to grant Microphone and Screen Recording again. Approve both to restore audio capture.'
  ...
  safeHandle('calendar-connect', async () => {
    try {
      ...
      if (!manager.isProxyConfigured()) {
        return {
          success: false,
          disabled: true,
          error: 'Calendar is disabled until an explicit proxy URL is configured.',
        };
      }
      ...
      return { success: false, disabled: /Calendar is disabled/i.test(error?.message || ''), error: error.message };
    }
  });
