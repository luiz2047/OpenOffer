 ...
 * dual-stack hosts can return a hard `ENOTFOUND` for IPv4-only endpoints when
 * the machine has a link-local IPv6 address (fe80::...) but no real v6 path.
 ...
 * Streaming STT endpoints are often IPv4-only at the edge today, so we lose
 * little by pinning the resolver here. If a vendor later moves to IPv6-only or
 * v6-preferred, swap to family=0 (AF_UNSPEC) with a custom v6-to-v4 fallback.
 */
