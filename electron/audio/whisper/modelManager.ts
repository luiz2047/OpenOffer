...
const MODEL_ID_ALIASES: Record<string, WhisperModelId> = {
  'onnx-community/whisper-large-v3-turbo': 'onnx-community/whisper-large-v3-turbo-ONNX',
};
...
const MODEL_CATALOG: WhisperModelInfo[] = [
  ...
  // ── Whisper Large v3 — quality-first multilingual fallback for Russian,
  // heavier than Turbo and best kept to Apple Silicon.
...
export function getModelSizeBytes(modelId: string): number {
  ...
  const m = MODEL_CATALOG.find(x => x.id === normalizedModelId);
