 ...
 * NativelyProSTT
 *
 * Retired hosted STT compatibility class.
 *
 ...
 *
 * The public OpenOffer baseline must never connect to a hosted Natively STT
 * relay. The compatibility endpoint below is intentionally local-only and
 * unreachable so accidental use fails closed instead of calling cloud infra.
 *
...
export class NativelyProSTT extends EventEmitter {
    ...
    private readonly BACKEND_URL = 'ws://127.0.0.1:9/removed-hosted-stt';
    ...
    constructor(
        ...
        this.deps    = deps;
        // Kept only for constructor compatibility with older tests.
        this.controlPlaneBaseUrl = deps.controlPlaneBaseUrl ?? this.deriveControlPlaneBase();
        this.appVersion = deps.appVersion ?? '';
        this.platform   = deps.platform ?? '';
    }

    /** Local discard endpoint derived from the inert BACKEND_URL. */
    ...
    /** No-op: retained for compatibility with the removed hosted STT provider. */
    ...
    private isOnRelayTarget(_url: string): boolean {
        return false;
    }
