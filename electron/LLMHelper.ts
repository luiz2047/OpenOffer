...
[REMINDER] Your entire response MUST be in ${lang} only. Never switch to English.`;
  ...
  private async * _streamChatInner(
    ...
    // which would dead-end when the selected model failed and only Gemini remained.
    // The text-only routing below is unchanged.
    ...
    // Local Groq key fast path: call Groq directly for short text-only answers.
    //
    ...
    if (fastModeApplies) {
      ...
      if (this.groqClient && !this._groqLocalDisabled) {
        ...
        // Local Groq failed — fall through to the standard routing below.
      }
    }
    ...
    if (this.client) {
      ...
      // instead of failing the whole answer: a dead Gemini key should not take
      // the live answer down when another provider is configured.
   ...
   * 1. Custom / cURL Provider (if user selected one)
