// tests/intelligence/e2e/OpenOfferIntelligence100Questions.test.mjs
//
// ============================================================================
...
//      (real provider) against a SAFE COPY of the real openoffer.db. This is the EXACT
...
//
// Run: node --test tests/intelligence/e2e/OpenOfferIntelligence100Questions.test.mjs
...
after(() => {
  ...
  const outPath = path.join(REPO_ROOT, 'openoffer-intelligence-e2e-results.json');
  const byCat = {};
...
for (const q of QUESTIONS.filter((x) => x.category === 'A' || x.category === 'B')) {
  test(`[${q.id}] ${q.category} manual — ${q.question}`, async () => {
    ...
    else if (scan.nativelyIdentityLeak) reason = 'WRONG IDENTITY — answer claims to be OpenOffer/an AI assistant on a candidate question';
...
for (const q of QUESTIONS.filter((x) => x.category === 'C')) {
  test(`[${q.id}] C WTA — ${q.question}`, async () => {
    ...
    else if (scan.nativelyIdentityLeak) reason = 'WRONG IDENTITY — "I\'m OpenOffer" in live copilot';
